...
export const NeverminedProvider = ({ children, config, verbose }: NeverminedProviderProps) => {
  ...
  const account: AccountModule = {
    ...
    getAssociatedServices: async (did: string, searchOptions?: SearchOptions): Promise<QueryResult> => {
      try {
        const query = await sdk.search.servicesBySubscription(did, searchOptions?.customNestedQueries, searchOptions?.offset, searchOptions?.page, searchOptions?.sort, searchOptions?.appId)
        return query
      } catch (error) {
        verbose && Logger.error(error)
        return emptyQueryResult
    ...
    getAssociatedDatasets: async (did: string, searchOptions?: SearchOptions): Promise<QueryResult> => {
      try {
        const query = await sdk.search.datasetsBySubscription(did, searchOptions?.customNestedQueries, searchOptions?.offset, searchOptions?.page, searchOptions?.sort, searchOptions?.appId)
        return query
      } catch (error) {
        verbose && Logger.error(error)
        return emptyQueryResult
  ...
  const assets: AssetsModule = {
    ...
    transfer: async ({ did, newOwner, amount, ercType }: { did: string; newOwner: Account; amount: number; ercType?: ERCType }): Promise<boolean> => {
      try {
    ...
    orderAsset: async (did: string, account: Account): Promise<string> => {
    ...
    orderNFT1155: async (did: string, account: Account, amount: BigNumber): Promise<string> => {
    ...
    orderNFT721: async (did: string, account: Account): Promise<string> => {
    ...
    getCustomErc20Token: async (customErc20TokenAddress: string, account: Account) => {
